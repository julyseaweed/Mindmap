...
export function relationshipGeometry(relationship: MindRelationship, boxes: Record<string, Box>, measure?: (text: string) => number): RelationshipGeometry | null;
export function relationshipBounds(layout: { boxes: Record<string, Box>; width: number; height: number }, relationships: MindRelationship[], measure?: (text: string) => number): RelationshipBounds;
